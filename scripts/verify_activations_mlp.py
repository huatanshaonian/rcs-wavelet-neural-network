...
print("=== 验证 MLP 模型激活函数配置 ===")

# 创建系统: High=SIN, Smooth=RELU, Architecture=MLP
print("\n1. 创建系统 (High=SIN, Smooth=RELU, Arch=additive_dual_branch_mlp)...")
try:
    system = create_autoencoder_system(
        config_name='2freq',
        mode='wavelet',
        architecture='additive_dual_branch_mlp',
        activation_high='sin',
    ...
    print("\n[检查 High 分支 (decoder_high) - 预期: SIN]")
high_mlp_acts = check_activation(ae.decoder_high)
print(f"  MLP层激活: {high_mlp_acts}")

is_high_correct = all('Sin' in name for name in high_mlp_acts)
print(f"  -> 结果: {'✅ 正确 (全是SIN)' if is_high_correct else '❌ 错误'}")

# 检查 Smooth 分支 (应该全是 ReLU)
print("\n[检查 Smooth 分支 (decoder_smooth) - 预期: RELU]")
smooth_mlp_acts = check_activation(ae.decoder_smooth)
print(f"  MLP层激活: {smooth_mlp_acts}")

is_smooth_correct = all('ReLU' in name for name in smooth_mlp_acts)
